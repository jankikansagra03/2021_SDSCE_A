...
<body>
    <form action="FetchRegister" method="post" enctype="multipart/form-data">
        ...
        <input type="checkbox" name="hobby[]" id="hobby1" value="Playing Sports"
            @if (is_array(old('hobby')) && in_array('Playing Sports', old('hobby'))) chec0ked @endif />Playing Sports
        <br>
        ...
        <br>
        Select Profile Picture: <input type="file" name="pic" id="pic1" value="{{ old('pic') }}">
        <br>
        <span style="color:red">
            @error('pic')
                {{ $message }}
